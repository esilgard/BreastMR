...
class Her2FISH(OneFieldPerReportML):
    ...
    def __init__(self):
        ...
        self.model = joblib.load(PATH + "models"+os.path.sep+"Her2FISH"+os.path.sep+"svm_model_window7skip2.pkl")
        self.feature_mapping = dict((x.split('\t')[0], int(x.strip().split('\t')[1])) for x in \
                               open(PATH + "models"+os.path.sep+"Her2FISH"+os.path.sep+"feature_mapping.txt", 'r').readlines())
        self.class_label_mapping = {0: None, 1: 'Equivocal', 2: 'Positive', 3: 'Negative',\
